...
def main():
	...
	server = SpeechASR(
		mic_device=rospy.get_param('~mic_device', 0),                     # Sound device to use as microphone (integer index or search for case-insensitive substring, see python -m sounddevice for list of devices)
...
class WhisperASR(ASRModel):
	...
	def __init__(self, variant='small', audio_lang='english', translate=True, preload_model=True, use_cuda=True, verbose=False):
		...
		device=torch.device('cpu') #NewAdd
