...
def main():
    ...
    with sm:
        ...
        #def llm_response_callback(userdata, response):
            ...
            #return "succeeded"
        def llm_response_callback(userdata, response):
            # Log the received response to help with debugging
            rospy.loginfo(f"Received response: {response.response}")
            if response.response is None or not response.response:
                rospy.logerr("Received empty or None response from service!")
                return "aborted"  # You can handle this failure as needed
            try:
                # Try parsing the response JSON
                actions = json.loads(response.response)["actions"]
                userdata.llm_actions = actions
                rospy.loginfo(f"llm_actions before callback: {actions}")

            except KeyError as e:
                rospy.logerr(f"Failed to extract 'actions' from response: {e}")
                return "aborted"  # Handle failure or retry logic as needed
            except json.JSONDecodeError as e:
                rospy.logerr(f"Failed to decode JSON response: {e}")
                return "aborted"  # Handle failure or retry logic as needed
            # Log the parsed actions for further debugging
            rospy.loginfo(f"Parsed LLM actions: {actions}")
            # Return "succeeded" if everything works correctly
            return "succeeded"
        smach.StateMachine.add(
            ...
            remapping={"prompt": "llm_input","llm_actions":"llm_actions",},
            transitions={
